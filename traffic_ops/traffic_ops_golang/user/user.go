...
func (user *TOUser) ParamColumns() map[string]dbhelpers.WhereColumnInfo {
	return map[string]dbhelpers.WhereColumnInfo{
		...
		"username": dbhelpers.WhereColumnInfo{"username", nil},
	}
}
...
func (user *TOUser) Validate() error {
	...
	if user.LocalPassword != nil && user.ConfirmLocalPassword != nil {
		...
		_, err := auth.IsGoodLoginPair(*user.LocalPassword, *user.Username)
		if err != nil {
			return err
		}
	}
...
func (user *TOUser) Read() ([]interface{}, error, error, int) {
	...
	for rows.Next() {
		...
		if checkTenancy(user.TenantID, tenantIDs) {
...
func (user *TOUser) UpdateQuery() string {
	...
	confirm_local_passwd=COALESCE(:confirm_local_passwd, confirm_local_passwd)
	WHERE id=:id
